        ...
        {(phase === 'prism' || phase === 'explode') && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-8">
            ...
            <div className="text-center animate-[popIn_0.8s_cubic-bezier(0.17,0.67,0.83,0.67)_0.5s_forwards] opacity-0">
              ...
              <p className="text-sm text-indigo-400 font-mono tracking-[0.3em] uppercase">AI สร้างภาพสินค้าตัวอย่าง</p>
            </div>
          </div>
        )}
...
const WorkflowStepper = ({ currentView, onViewChange }: { currentView: AppView, onViewChange: (view: AppView) => void }) => {
  const steps = [
    { id: 'assets', label: 'อัปโหลดรูปภาพ', number: 1 },
    { id: 'studio', label: 'ออกแบบ', number: 2 },
    { id: 'gallery', label: 'ดาวน์โหลด', number: 3 },
  ];
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs bg-zinc-800 px-2 py-1 rounded text-zinc-400">{assets.length} รายการ</span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6 overflow-y-auto max-h-[400px] pr-2">
        ...
        {assets.length === 0 && (
          <div className="col-span-2 sm:col-span-3 flex flex-col items-center justify-center h-32 text-zinc-500 border border-dashed border-zinc-800 rounded-lg">
            <p className="text-sm">ไม่มี {type === 'product' ? 'สินค้า' : 'โลโก้'}</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-zinc-800">
        ...
        {mode === 'upload' ? (
          <FileUploader label={`อัปโหลด ${type === 'product' ? 'สินค้า' : 'โลโก้'}`} onFileSelect={(f) => {
          ...
          <div className="space-y-3">
            <textarea
              ...
              placeholder={`อธิบายลักษณะของ ${type === 'product' ? 'สินค้า' : 'โลโก้'} ที่คุณต้องการ...`}
            ...
            >
              สร้าง {type === 'product' ? 'สินค้า' : 'โลโก้'}
            </Button>
          </div>
        )}
      </div>
...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading({ isGenerating: true, message: 'กำลังวิเคราะห์รูปทรง...' });
    try {
      ...
      <aside className="w-64 border-r border-zinc-800 bg-zinc-950/50 hidden md:flex flex-col">
        ...
        <div className="p-4 border-t border-zinc-800">
          <div className="p-4 rounded-lg bg-zinc-900/50 border border-zinc-800 text-center">
            <Button size="sm" variant="outline" className="w-full text-xs">เอกสารคู่มือ</Button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto relative pt-16 md:pt-0">
        ...
        <div className="sticky top-0 z-40 h-16 bg-black/80 backdrop-blur-md border-b border-zinc-800 flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-4">
            <Button size="sm" variant="ghost" icon={<Sparkles size={16} />}>เครดิต: ∞</Button>
          </div>
        </div>
            ...
            <div className="animate-fade-in space-y-8">
              <div className="text-center py-12">
                <h1 className="text-4xl md:text-6xl font-black mb-6 text-white">
                  สร้างภาพตัวอย่างสินค้าที่ <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-500">สมจริงและสวยงาม</span>
                </h1>
                <p className="text-zinc-400 text-lg max-w-2xl mx-auto mb-10">
                  อัปโหลดโลโก้และสินค้าของคุณ เพื่อให้ AI ของเราผสานภาพอย่างสมจริงทั้งแสงเงาและการวางตำแหน่ง
                </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { icon: <Box className="text-indigo-400" />, title: 'จัดการรูปภาพ', desc: 'จัดระเบียบโลโก้และรูปสินค้า' },
                  { icon: <Wand2 className="text-purple-400" />, title: 'ผสานภาพด้วย AI', desc: 'การผสมผสานและการวางพื้นผิวอัจฉริยะ' },
                  { icon: <Download className="text-pink-400" />, title: 'ส่งออกความละเอียดสูง', desc: 'ภาพพร้อมใช้งานจริง' }
                ].map((feat, i) => (
              ...
              <footer className="mt-20 pt-8 border-t border-zinc-900 text-center">
                <p className="text-white text-sm max-w-2xl mx-auto leading-relaxed">
                  "การใช้งานแอปนี้แสดงว่าคุณยืนยันว่าเป็นเจ้าของลิขสิทธิ์เนื้อหาที่คุณอัปโหลด ห้ามสร้างเนื้อหาที่ละเมิดทรัพย์สินทางปัญญาหรือสิทธิความเป็นส่วนตัวของผู้อื่น การใช้บริการ AI นี้อยู่ภายใต้นโยบายการใช้งานที่ห้ามใช้ในทางที่ผิด
                  <br className="hidden md:block" />
                  โปรดทราบว่าการอัปโหลดจาก Google Workspace อาจถูกนำไปใช้เพื่อพัฒนาและปรับปรุงผลิตภัณฑ์และบริการของ Google ตามข้อกำหนดของเรา"
                </p>
              </footer>
            </div>
          )}
          ...
          {view === 'studio' && (
            <div className="animate-fade-in h-[calc(100vh-8rem)] md:h-[calc(100vh-12rem)] flex flex-col-reverse lg:flex-row gap-4 lg:gap-6">
              ...
              <div className="w-full lg:w-80 flex flex-col gap-6 glass-panel p-6 rounded-2xl overflow-y-auto flex-1 lg:flex-none">
                <div>
                  <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider mb-4">1. เลือกสินค้า</h3>
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    {assets.filter(a => a.type === 'product').length === 0 && <p className="text-xs text-zinc-400 col-span-3">ยังไม่มีสินค้า</p>}
                  </div>
                </div>

                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider">2. เพิ่มโลโก้</h3>
                    {placedLogos.length > 0 && (
                      <span className="text-xs text-indigo-400">{placedLogos.length} บนภาพ</span>
                    )}
                  </div>
                  <p className="text-xs text-zinc-400 mb-2">คลิกเพื่อเพิ่ม ลากเพื่อย้าย เลื่อนเมาส์เพื่อย่อขยาย</p>
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    {assets.filter(a => a.type === 'logo').length === 0 && <p className="text-xs text-zinc-400 col-span-3">ไม่มีโลโก้</p>}
                  </div>
                </div>

                <div>
                  <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider mb-4">3. คำสั่งเพิ่มเติม</h3>
                  <textarea
                    ...
                    placeholder="เช่น ทำให้โลโก้เนียนไปกับพื้นผิวผ้า..."
                    value={prompt}
              ...
              <div className="h-[45vh] lg:h-auto lg:flex-1 glass-panel rounded-2xl flex items-center justify-center bg-zinc-900 relative overflow-hidden select-none flex-shrink-0">
                ...
                {selectedProductId ? (
                  ...
                  <div className="text-center text-zinc-600">
                    ...
                    <p>เลือกสินค้าเพื่อเริ่มออกแบบ</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {view === 'gallery' && (
            <div className="animate-fade-in">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-bold">ภาพที่สร้างเสร็จแล้ว</h2>
                <Button variant="outline" onClick={() => setView('studio')} icon={<Plus size={16} />}>สร้างใหม่</Button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="aspect-square bg-zinc-900 relative overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        ...
                        <a href={mockup.imageUrl} download={`mockup-${mockup.id}.png`}>
                          <Button size="sm" variant="primary" icon={<Download size={16} />}>บันทึก</Button>
                        </a>
                      </div>
                    </div>
                    <div className="p-4">
                      <p className="text-xs text-zinc-500 mb-1">{new Date(mockup.createdAt).toLocaleDateString()}</p>
                      <p className="text-sm text-zinc-300 line-clamp-2">{mockup.prompt || "ภาพที่สร้างโดยอัตโนมัติ"}</p>
                      {mockup.layers && mockup.layers.length > 0 && (
                        <div className="mt-2 flex gap-1">
                          <span className="text-xs px-1.5 py-0.5 bg-zinc-800 rounded text-zinc-400">{mockup.layers.length} โลโก้</span>
                        </div>
                      )}               </div>
                  </div>
                ))}
                {generatedMockups.length === 0 && (
                  <div className="col-span-full py-20 text-center glass-panel rounded-xl">
                    ...
                    <h3 className="text-lg font-medium text-zinc-300">ยังไม่มีภาพที่สร้าง</h3>
                    <p className="text-zinc-500 mb-6">เริ่มสร้างภาพแรกของคุณในสตูดิโอ</p>
                    <Button onClick={() => setView('studio')}>ไปที่สตูดิโอ</Button>
                  </div>
                )}
              </div>
            </div>
